...
const StarRating = () => {
  ...
  const toggleActive = useCallback(() => setShowModal((showModal) => !showModal), []);
  ...
  const handleStarHover = (star) => {
    const updatedColors = starColors.map((_, index) => (star >= index + 1 ? "yellow" : "gray"));
    setStarColors(updatedColors);
  };

  const handleStarLeave = () => {
    const updatedColors = starColors.map((_, index) => (index + 1 <= rating ? "yellow" : "gray"));
    setStarColors(updatedColors);
  };

  const handleStarModalHover = (star) => {
    const updatedColors = starModalColors.map((_, index) => (star >= index + 1 ? "yellow" : "gray"));
    setStarModalColors(updatedColors);
  };

  const handleStarModalLeave = () => {
    const updatedColors = starModalColors.map((_, index) => (index + 1 <= modalRating ? "yellow" : "gray"));
    setStarModalColors(updatedColors);
  };
  ...
  return (
    <Banner onDismiss={() => {}}>
      <div className="inline">
        <div>
          <p>We believe you had the best time here. Please let us know how's your experience. It means a lot to us.</p>
        </div>
        <div>
          ...
          {showModal && (
            ...
            >
              <Modal.Section>
                ...
                <TextField value={modaltext} onChange={handleChange} multiline={5} maxLength={1000} autoComplete="off" />
              </Modal.Section>
            </Modal>
          )}
        </div>
