import {Button,Popover, IndexTable, Icon,InlineStack,Text} from '@shopify/polaris';
import {ParentContext} from "../ProductTable/ProductTable"
import React, {useState, useCallback,useEffect,useContext} from 'react';
import {
    SettingsMinor,DragHandleMinor
...
function CustomizedTable({updateParentValue,ordersRows}) {
  ...
  const activator = <Button onClick={handleChange} size='large' icon={SettingsMinor}>
  </Button>;
    ...
    .map(obj => ({ title: obj.title }));
     const updatedRow = ordersRows.map((row,index) => (
      ...
      >   
        ...
        {item.title === "ProName" ? (row[item.title].length > 15 ? row[item.title].slice(0, 20) + '....' : row[item.title]): row[item.title]}
